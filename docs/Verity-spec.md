...
- pin dstack version ≥ 0.5.6 (post attestation-pipeline hardening). Attestation structure **measured against 0.5.7**, which Phala **no longer offers** — 0.5.8 and 0.5.9 are current as of 2026-08-08. State continuity was re-verified 2026-08-08 with guest image 0.5.9, **on nodes still running v0.5.7** ([record](../records/experiments/2026-08-08-l02-l03-continuity-on-dstack-059.md), [correction](../records/experiments/2026-08-08-correction-guest-image-is-not-the-node-version.md)); and the attestation structure (`MR-CONFIG-ID` V1, quote layout) re-confirmed by L-04 the same day on guest image 0.5.9 ([record](../records/experiments/2026-08-08-l04-on-dstack-059.md)). `boot_measurements` remains unexercised against real hardware on any version
...
- **I4.** Payment and license mint are one act from the agent's perspective: the 402-gated resource **is** the mint authorization, and no authorization exists for a payment that has not settled on chain. ⟳ A lost response does not lose the entitlement — `/purchase` is idempotent on the payment authorization and re-issues an authorization carrying **the same mint nonce** (a fresh expiry, and therefore a different signature) against the settled transfer read from chain, closing the window with the buyer's own retry rather than a queue, a webhook or an operator. Because both the payment nonce and the mint nonce are spent in on-chain state, one settled payment yields **at most one licence** however many times it is re-issued. *Atomicity here is a recoverability claim, not a single-transaction claim, and recovery is bounded by the conditions in [ADR 0031](decisions/0031-purchase-idempotency-is-chain-derived.md) (C2, C3); re-verify if the payment path moves to ERC-7710 (§4.2).*
